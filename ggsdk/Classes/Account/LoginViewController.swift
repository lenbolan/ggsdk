...
class LoginViewController: UIViewController {
    ...
    private let btnToRegist: UIButton = {
        let btn = UIButton()
        
        var attributedString = NSMutableAttributedString(string: "没有账号？前往注册")
        var range = NSRange()
        range.location = 7
        range.length = 2
    ...
    override func viewDidLoad() {
        ...
        lbPhone.anchor(top: lbSubtitle.bottomAnchor, left: view.leftAnchor, bottom: nil, right: nil, paddingTop: 100, paddingLeft: 40, paddingBottom: 0, paddingRight: 0, width: Util.screenWidth-80, height: 20)
        ...
        lbPwd.anchor(top: txtPhone.bottomAnchor, left: view.leftAnchor, bottom: nil, right: nil, paddingTop: 30, paddingLeft: 40, paddingBottom: 0, paddingRight: 0, width: Util.screenWidth-80, height: 20)
        ...
        btnLogin.anchor(top: txtPwd.bottomAnchor, left: view.leftAnchor, bottom: nil, right: nil, paddingTop: 40, paddingLeft: 40, paddingBottom: 0, paddingRight: 0, width: Util.screenWidth-80, height: 40)
        btnToRegist.anchor(top: btnLogin.bottomAnchor, left: nil, bottom: nil, right: nil, paddingTop: 10, paddingLeft: 0, paddingBottom: 0, paddingRight: 0, width: 200, height: 20)
        btnToRegist.centerXAnchor.constraint(equalTo: view.centerXAnchor).isActive = true
        ...
        btnToRegist.addTarget(self, action: #selector(onTapToRegist), for: .touchUpInside)
        
        setKefuUI()
        
        let vLogin = UIView(frame: CGRect(x: Util.screenWidth-64, y: 0, width: 64, height: 64))
//        vLogin.borderWith = 1
//        vLogin.borderColor = .red
        let doubleTap = UITapGestureRecognizer(target: self, action: #selector(onDoubleTap(tap:)))
        doubleTap.numberOfTapsRequired = 2
        vLogin.addGestureRecognizer(doubleTap)
        view.addSubview(vLogin)
    }
    
    @objc
    private func onDoubleTap(tap:UITapGestureRecognizer) {
        doLogin(phone: "15821162092", pwd: "qqqqqq")
    }
    ...
    func onTapLogin() {
        ...
        doLogin(phone: phone, pwd: pwd)
    }
    
    private func doLogin(phone: String, pwd: String) {
        let parameters = ["username": phone, "password": pwd]
        Alamofire.request(adLogin, method: .post, parameters: parameters).responseJSON { [self] response in
            Log.debug(response)
            if let data = response.data {
                let json = JSON(data)
                if json["code"].int == 0 {
                    view.makeToast("登录成功")
                    UserDefaults.localData.keyAccount.store(value: phone)
                    
                    if let uid = json["data"]["uid"].int,
                       let token = json["data"]["token"].string {
                        rAdInit(uid: uid, phoneNum: phone, token: token)
                    }
                    
                    rAdBaseInfo()
                    
                    self.dismiss(animated: false, completion: nil)
                    NotificationCenter.default.post(name: NSNotification.Name(NotiName.openAd.rawValue), object: nil)
    ...
    @objc
    private func onTapToRegist() {
        self.dismiss(animated: false, completion: nil)
        NotificationCenter.default.post(name: NSNotification.Name(NotiName.openRegistView.rawValue), object: nil)
    }
